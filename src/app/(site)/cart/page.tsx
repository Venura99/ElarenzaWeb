...
export default function CartPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex items-start gap-3 sm:flex-1 sm:items-center sm:gap-4">
              <div className="h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-cream-soft sm:h-20 sm:w-20">
                {item.image ? (
                  <Image
                    src={item.image}
                    alt={item.name}
              ...
              <button
                ...
                className="-mt-1 shrink-0 px-1 text-xl leading-none text-red-500 hover:text-red-700 sm:hidden"
              >
                ×
              </button>
            </div>
            ...
            <div className="flex items-center justify-between gap-3 sm:justify-end sm:gap-4">
              ...
              <p className="font-medium text-ink sm:w-24 sm:text-right">
              ...
              <button
                ...
                onClick={() => removeItem(item.variantId)}
                aria-label={`Remove ${item.name} from cart`}
                className="hidden shrink-0 px-1 text-xl leading-none text-red-500 hover:text-red-700 sm:block"
              >
                ×
              </button>
            </div>
      ...
      <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-gold-light/40 bg-cream-soft p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
        ...
        <Link
          href="/checkout"
          className="rounded-full bg-ink px-8 py-3 text-center text-sm font-semibold uppercase tracking-wide text-cream hover:bg-gold-dark"
        >
          Proceed to Checkout
        </Link>
      </div>
